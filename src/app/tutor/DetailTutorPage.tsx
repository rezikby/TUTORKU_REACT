...
export default function DetailTutorPage(props: any) {
  ...
  const parseGoogleMapsInfo = (url?: string | null) => {
    if (!url) return null;
    try {
      ...
      const dMatch = normalized.match(/!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/);
      const coords = atMatch
        ? { lat: atMatch[1], lng: atMatch[2] }
        : dMatch
        ? { lat: dMatch[1], lng: dMatch[2] }
        : null;
  ...
  const googleMapsInfo = parseGoogleMapsInfo(tutor?.google_maps_url);
  const resolvedLocation = googleMapsInfo?.placeName || tutor?.location || tutor?.city || tutor?.province || t("detailTutor.online");
  const mapUrl = googleMapsInfo?.coords
    ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${googleMapsInfo.coords.lat},${googleMapsInfo.coords.lng}`)}`
    : null;
              ...
              {mapUrl && (
                <div className="mt-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-blue-600 hover:underline text-xs xs:text-sm"
                  >
                    <MapPin size={14} /> {t("detailTutor.openMaps")}
                  </a>
                </div>
              )}
                ...
                {mapUrl && (
                  <div className="mt-1">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-0.5 text-blue-600 hover:underline text-[10px] xs:text-xs"
                    >
                      <MapPin size={12} /> {t("detailTutor.openMaps")}
                    </a>
                  </div>
                )}
              </div>
