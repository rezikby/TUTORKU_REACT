...
function TutorCardUI({ tutor, onView, onBook }: { tutor: any; onView: () => void; onBook: () => void }) {
  ...
  const locationMapUrl = tutor.latitude != null && tutor.longitude != null
    ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${tutor.latitude},${tutor.longitude}`)}`
    : tutor.location
      ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(tutor.location)}`
      : null;
